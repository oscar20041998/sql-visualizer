...
**Mandatory Security Requirement**: ALL API endpoints in this system MUST be secured. Every request must include a valid JWT token in the header:
...
| `PUT` | `/api/user/profile` | Update profile | `displayName`, `avatarUrl` | `200 OK` |
| `GET` | `/api/user/settings` | Get settings | N/A | `200 OK` (`SettingsDTO`) |
| `PUT` | `/api/user/settings` | Save settings | `theme`, `defaultDialect` | `200 OK` |
...
| Field | Java Type | TS Type | Description |
| :--- | :--- | :--- | :--- |
| `level` | `String` | `'LOW' \| 'MEDIUM' \| 'HIGH' \| 'VERY_HIGH'` | Complexity severity classification. |
...
| `origin` | `String` | `string` | Yes | Direct physical parent table column name if resolvable. |
...
```json
{
  ...
  "executionCost": {
    ...
    "maxScore": 100.0,
    "factors": [
      { "name": "Indexed Join Check", "impact": "low", "note": "Primary key join u.id = o.user_id detected." }
    ],
...
Used to read, save, and search user execution history. Leverages `pgvector` on the PostgreSQL DB for sub-second semantic searches.
...
*   **Response Payload (Status `201 Created`):**
    ```json
    {
      ...
      "joinCount": 1,
      "complexityLevel": "LOW",
      "embedding": [0.0123, -0.0456, 0.0789, "... (1536 float values)"]
    }
    ```

#### B. Search/List Query History (`GET /api/history`)
Lists persistent query runs. If `query` parameter is supplied, executes a semantic cosine distance calculation over vector embeddings.
...
*   **Response Payload (Status `200 OK`):**
    Returns a `List<QueryHistoryEntry>` matching the schema returned by the POST action, sorted by similarity score (if searched) or creation date descending.
